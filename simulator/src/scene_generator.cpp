...
void SceneGenerator::GenerateLandmarks(int num_landmarks, Scene &scene) {
  ...
  cv::Vec3d center(0, 0, 5);
  cv::RNG rng;
  cv::Mat_<double> points3d;
  // Generate a bunch of random 3d points in a cube
  points3d.create(num_landmarks, 3);
  rng.fill(points3d, cv::RNG::UNIFORM, -2, 2);
  // Copy to scene.
  for (int i = 0; i < num_landmarks; ++i) {
    scene.landmarks[i].position[0] = points3d.at<double>(i, 0) + center[0];
    scene.landmarks[i].position[1] = points3d.at<double>(i, 1) + center[1];
    scene.landmarks[i].position[2] = points3d.at<double>(i, 2) + center[2];
  }
}
...
void SceneGenerator::GenerateViews(int num_views, Scene &scene) {
  ...
  for (int i = 1; i < num_views; ++i) {
    ...
    vec = vec / cv::norm(vec) * rng.uniform(0.0f, float(CV_PI) / 6.0f);
    cv::Rodrigues(vec, scene.trajectory[i].R);
    // scene.trajectory[i].R = cv::Mat::eye(3, 3, CV_64F);

    // ----------------- Add random translation
    scene.trajectory[i].t =
        cv::Vec3d(rng.uniform(-0.3f, 0.3f), rng.uniform(-0.3f, 0.3f),
